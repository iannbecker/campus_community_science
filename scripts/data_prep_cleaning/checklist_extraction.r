##############################
#
# eBird Checklist Extraction
# College Campus Community Science Study
# Updated methodology with OSM campus boundaries
#
# PROCESSING WORKFLOW:
# - Direct read from raw EBD files (no intermediate files)
# - All filters applied in memory:
#   1. Complete checklists only
#   2. Date range 2015-2024
#   3. Unique checklists (one per sampling event)
#   4. Travel distance ≤20km
# - Memory cleanup after each state
#
##############################

# This is a cluster script to extract ebird data for counting checklists
(OUT OF DATE) -see cluster 
# Libraries
library(auk)
library(dplyr)
library(sf)
...
# eBird data files - UPDATE THESE PATHS
...
##############################
# STEP 3: Process each state (direct read and filter)
##############################
...
process_state <- function(state_name, state_code) {
  ...
  # auk_ebd() creates a reference to the files
  # read_ebd() loads them into memory
...
output_rds <- file.path(output_dir, "ebird_combined_2015_2024.rds")
saveRDS(ebird_combined, output_rds)
cat("\nSaved combined dataset to:", output_rds, "\n")

# Also save as CSV (may be large)
output_csv <- file.path(output_dir, "ebird_combined_2015_2024.csv")
